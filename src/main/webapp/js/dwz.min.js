...
$("a[target=openwin]",$p).each(function(){
	$(this).click(function(event){
...
this._closeTimer=setTimeout(function(){alertMsg.close()},800);}else{
$(this._bgId).show();}
...
//zhoujd修改Begin
if(_lookup['$target'] && _lookup['$target'].parent() && _lookup['$target'].parent().parent()
		&& _lookup['$target'].parent().parent().get()[0] && _lookup['$target'].parent().parent().get()[0].tagName
		&& _lookup['$target'].parent().parent().get()[0].tagName == "TD") { //行项选择
	$box=_lookup['$target'].parent().parent().parent();
}
//zhoujd修改End
...
//zhoujd修改Begin
if(name.indexOf(".")>-1) {// 将manuLookup.manuid转化为map[manuid]
	name = "map[" + name.split(".")[1] + "]";
}
//zhoujd修改End
if(name==inputName){
$input.val(args[key]);
...
$this.click(function(event){
$this = $(event.target); //this会默认为隐藏的复制行
